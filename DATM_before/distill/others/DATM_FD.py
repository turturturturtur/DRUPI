...
def main(args):
    ...
    feat_syn = torch.randn(size=(num_classes * args.ipc, channel, im_size[0], im_size[1]), dtype=torch.float)
    ...
    ''' training '''
    feat_syn = feat_syn.detach().to(args.device).requires_grad_(True)
    syn_lr = syn_lr.detach().to(args.device).requires_grad_(True)
    
    optimizer_img = torch.optim.SGD([feat_syn], lr=args.lr_img, momentum=0.5)
    ...
    '''------test------'''
    '''test the initial accuracy of synthetic data'''
    ...
    for it in range(0, args.Iteration+1):
        ...
        if it in eval_it_pool:
            for model_eval in model_eval_pool:
                ...
                for it_eval in range(args.num_eval):
                    ...
                    with torch.no_grad():
                        image_save = feat_syn.to(device)
                    feat_syn_eval, label_syn_eval = copy.deepcopy(image_save.detach()).to(device), copy.deepcopy(eval_labs.detach()).to(device) # avoid any unaware modification

                    args.lr_net = syn_lr.item()
                    _, acc_train, acc_test = evaluate_synset(it_eval, copy.deepcopy(net_eval).to(device), feat_syn_eval.to(device), label_syn_eval.to(device), testloader, args, texture=False, train_criterion=criterion)
        ...
        if it in eval_it_pool and (save_this_it or it % 1000 == 0):
            with torch.no_grad():
                ...
                wandb.log({"Pixels": wandb.Histogram(torch.nan_to_num(feat_syn.detach().cpu()))}, step=it)
