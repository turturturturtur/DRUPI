...
def main(args):
    ...
    label_syn = torch.tensor(
        [[i] * args.ipc for i in range(num_classes)],
        dtype=torch.long,
        requires_grad=False,
        device=args.device,
    ).view(
        ...
        for c in range(num_classes):
            ...
            while len(index) < args.ipc:
                ...
                if batch_size * select_times > len(data_for_class_c):
                    select_times = 0
                    cur += 1
                    temp_params = torch.load(label_expert_files[int(cur / 10) % 10])[
                        cur % 10
                    ][args.Label_Model_Timestamp]
                    temp_params = torch.cat(
                        [p.data.to(device).reshape(-1) for p in temp_params], 0
                    ).to(device)
                    if args.distributed and args.parall_eva == True:
                        temp_params = temp_params.unsqueeze(0).expand(
                ...
                if len(index) == args.ipc:
                    temp_img = torch.index_select(
                        data_for_class_c, dim=0, index=torch.tensor(index)
                    )
                    break
    ...
    for it in range(0, args.Iteration + 1):
        ...
        for step in range(args.syn_steps):
            ...
            grad = torch.autograd.grad(
                ce_loss, forward_params, create_graph=True, retain_graph=True
            )[0]
        ...
        for i in range(args.syn_steps):
            ...
            grad_i = torch.autograd.grad(
                ce_loss_i, w_i, create_graph=True, retain_graph=True
            )[0]
            ...
            gradients_x, gradients_y = torch.autograd.grad(
                total_term, [original_x_list[i], original_y_list[i]]
            )
            with torch.no_grad():
        ...
        wandb.log({"Grand_Loss": grand_loss.detach().cpu(), "Start_Epoch": start_epoch})
