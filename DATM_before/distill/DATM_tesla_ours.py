...
# 
    ...
    if args.pix_init == 'real':
        ...
        for _ in range(args.n_feat):
            ...
            optimizer_pretrained = torch.optim.SGD(pretrained_net.parameters(), lr=args.lr_teacher) 
...
# 
        net_parameters_wo_classifier = [param for name, param in student_net.named_parameters() if 'classifier' not in name]
...
# 
        ...
        # syn_feat_gradients = torch.zeros(feature_syn.shape).to(args.device)
        x_list = []
        original_x_list = []
        y_list = []
        original_y_list = []
        ...
        for step in range(args.syn_steps):
            ...
            original_feat_list.append(this_feat)
            optimizer_feat.zero_grad()
            ...
            feat_list.append(this_feat.clone())
            ...
            grad = torch.autograd.grad(loss_syn, forward_params, create_graph=True, retain_graph=True)[0]
            ...
            # acc是不是累加？
            with torch.no_grad():
                acc_feat_total += torch.sum(torch.argmax(output_feat_syn.detach().clone(), dim=1) == this_y).item()
