...
def main():
    ...
    model_args = f'{args.dataset}_{args.model}_{args.ipc}ipc_[{args.feat_metric}{args.lbd}_pool{args.pooling}_layer{args.layer_idx}_CE{args.feat_lbd}_opt{args.feat_opt}_nfeat{args.n_feat}_use{args.use_feature}_norm{args.feat_norm}]_{args.img_method}_{get_time()}_poolingFlag{args.poolFlag}__lr-feat{args.lr_feat}_imgUdp{args.imgUdp}'
    # if args.contrast:
        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool:
                for model_eval in model_eval_pool:
                    ...
                    final_acc.append((np.mean(accs),np.std(accs)))
    ...
    for key in model_eval_pool:
        ...
        accs = accs.reshape(-1)
        max_tuple = max(final_acc, key=lambda x: x[0])
        print('Run %d experiments with Strategy %s, train on %s, evaluate %d random %s, mean  = %.2f%%  std = %.2f%%'%(args.num_exp, feature_strategy,args.model, len(accs), key,max_tuple[0]*100 , max_tuple[1]*100))
