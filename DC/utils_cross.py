...
def epoch_with_features(
    ...
    for i, (img, lab, feature_syn) in enumerate(dataloader):
        ...
        if feature.size(1)!=feature_syn.size(1):           
            fc1=nn.Linear(feature.size(1), feature_syn.size(1)).to(args.device)
            feature = feature.permute(0, 2, 3, 1).to(args.device)
            feature=fc1(feature).to(args.device)
            feature = feature.permute(0, 3, 1, 2).to(args.device)
