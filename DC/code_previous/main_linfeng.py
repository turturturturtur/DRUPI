...
from utils import get_loops, get_dataset, get_network, get_eval_pool, evaluate_synset, get_daparam, match_loss, get_time, TensorDatasetFeature, epoch_with_features, DiffAugment, ParamDiffAug, get_all_features, FlushFile
import sys
...
def main():
    ...
    parser.add_argument('--data_path', type=str, default='/mnt/workspace/wangshaobo/data', help='dataset path')
    ...
    for exp in range(args.num_exp):
        ...
        # TODO: make sure this network is ConvNet
        feature_syn = torch.randn(size=(num_classes*args.ipc, channel, 4, 4), dtype=torch.float, requires_grad=True, device=args.device)
        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool:
                ...
                save_name = os.path.join(args.save_path, 'vis_feat_%s_%s_%s_%dipc_exp%d_iter%d_lbd%.3f.png'%(args.method, args.dataset, args.model, args.ipc, exp, it,args.lbd))
                feature_syn_vis = copy.deepcopy(feature_syn.detach().cpu())
                for ch in range(channel):
                    feature_syn_vis[:, ch] = feature_syn_vis[:, ch]  * std[ch] + mean[ch]
                feature_syn_vis[feature_syn_vis<0] = 0.0
                feature_syn_vis[feature_syn_vis>1] = 1.0
                save_image(feature_syn_vis, save_name, nrow=args.ipc) # Trying normalize = True/False may get better visual effects.
            ...
            for ol in range(args.outer_loop):
                ...
                for il in range(args.inner_loop):
                    epoch_with_features('train', trainloader, net, optimizer_net, criterion, feature_criterion, args, aug = True if args.dsa else False, lbd=args.lbd)
