...
def main():
    ...
    parser.add_argument('--lr_feat', type=float, default=0.01, help='learning rate for updating synthetic features')
        ...
        for it in range(args.Iteration+1):
            ...
            for ol in range(args.outer_loop):
                ...
                for c in range(num_classes):
                    ...
                    # print('feat_syn requires grad', feat_syn.requires_grad)
            ...
            loss_avg_feature /= (num_classes*args.outer_loop)
            images_norm.append(torch.norm(image_syn).item())
            features_norm.append(torch.norm(feature_syn).item())
            if it % 50 == 0:
                print('%s iter = %04d, loss = %.4f, loss_feat = %.4f' % (get_time(), it, loss_avg, loss_avg_feature))
            if it % 10 == 0:
