...
def main():
    ...
    parser.add_argument('--ckpt_path', default='../distilled_data', help='ckpt path')
    ...
    for exp in range(args.num_exp):
        ...
        if args.init == 'real':
            ...
            indices = torch.load(ckpt_path)['subset']['indices']
            images_temp = []

            for idx in indices:
                image, label = dst_train[idx]
                images_temp.append(image)

            # 将图片和标签转换为张量
            image_syn_pretrained = torch.stack(images_temp).to(args.device)
            if args.imgUdp:
            ...
            print('============================')
            print('image_syn  feature_syn',image_syn.shape,feature_syn.shape)
        else:
