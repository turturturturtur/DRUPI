...
''' Swish activation '''
class Swish(nn.Module): # Swish(x) = x?¦Ò(x)
    def __init__(self):
        super().__init__()
...
class MLP(nn.Module):
    ...
    def get_features(self, x, layer_idx=None):
        ...
        if layer_idx is None:
            return self.features(out).reshape(out.size(0), 128, 1, 1)  # Return features up to the final layer
        ...
        return out.reshape(out.size(0), 128, 1, 1)  
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, channel=3, num_classes=10, norm='instancenorm'):
        ...
        self.features = nn.Sequential(
            ...
            self.layer3, self.layer4, nn.AvgPool2d(kernel_size=4)
        )
        self.num_classes=num_classes
