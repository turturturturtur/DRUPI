...
def main(args):
    ...
    eval_it_pool = np.arange(
        (args.Iteration // 10) * 9, args.Iteration + 1, args.eval_it
    ).tolist()
    (
        channel,
        im_size,
    ...
    image_syn = torch.randn(
        size=(
            num_classes,
            args.ipc,
            1 if args.single_channel else 3,
            im_size[0],
            im_size[1],
        ),
        dtype=torch.float,
    ...
    styles = nn.ModuleList(
        [
            StyleTranslator(
                in_channel=1 if args.single_channel else 3,
                mid_channel=channel,
                out_channel=channel,
                kernel_size=3,
            )
    ...
    if args.pix_init == "real":
        ...
        for c in range(num_classes):
            if args.single_channel:
                image_syn.data[c] = (
                    get_images(c, args.ipc).detach().data.mean(dim=1, keepdim=True)
                )
            else:
    ...
    optimizer_style = torch.optim.SGD(
        styles.parameters(), lr=args.lr_style, momentum=0.95
    )
    optimizer_sim_content = torch.optim.SGD(
        sim_content_net.parameters(), lr=0.001, momentum=0.9
    )
    ...
    for it in range(0, args.Iteration + 1):
        ...
        if it in eval_it_pool and (save_this_it or it % 1000 == 0):
            with torch.no_grad():
                ...
                save_dir = os.path.join(
                    ".", "logged_files", args.dataset, wandb.run.name
                )
                ...
                torch.save(
                    styles.state_dict(),
                    os.path.join(save_dir, "styles_{}.pt".format(it)),
                )

                if save_this_it:
                    torch.save(
                        image_save.cpu(), os.path.join(save_dir, "images_best.pt")
                    )
                    torch.save(
                        styles.state_dict(), os.path.join(save_dir, "weights_best.pt")
                    )

                wandb.log(
                    {"Pixels": wandb.Histogram(image_syn.detach().cpu())}, step=it
                )
                ...
                if args.ipc < 50 or args.force_save:
                    ...
                    wandb.log(
                        {"Synthetic_Images": wandb.Image(grid.detach().cpu())}, step=it
                    )
                    wandb.log(
                        {
                            "Synthetic_Pixels": wandb.Histogram(
                                image_save.detach().cpu()
                            )
                        },
                        step=it,
                    )
        ...
        club_content_loss = (
            (
                torch.nn.functional.cosine_similarity(
                    embed_c[: base.shape[0]], embed_c[base.shape[0] :]
                )
                + 1.0
            )
            / 2.0
        ).mean() * args.lambda_club_content
        ...
        cls_content_loss = (
            criterion(logits_c, torch.cat([label, label], dim=0))
            * args.lambda_cls_content
        )
        likeli_content_loss = (
            (
                1
            ...
            / 2.0
        ).mean() * args.lambda_likeli_content
        embed_c_0 = torch.nn.functional.normalize(embed_c[: base.shape[0]])
        embed_c_1 = torch.nn.functional.normalize(embed_c[base.shape[0] :])
        print(embed_c_0.shape, embed_c_1.shape)
        print(label)
        exit()
        contrast_content_loss = (
            contrast(torch.stack([embed_c_0, embed_c_1], dim=1), label)
            * args.lambda_contrast_content
        )
        sim_content_loss = (
            cls_content_loss + likeli_content_loss + contrast_content_loss
        )
        ...
        if it % 10 == 0:
            print(
                "%s iter = %04d, loss = %.4f, param_loss = %.4f, club_content_loss = %.4f, sim_content_loss = %.4f, cls_content_loss = %.4f, likeli_content_loss = %.4f, contrast_content_loss = %.4f"
                % (
                    get_time(),
                    it,
                    grand_loss.item(),
                    param_loss.item(),
                    club_content_loss.item(),
                    sim_content_loss.item(),
                    cls_content_loss.item(),
                    likeli_content_loss.item(),
                    contrast_content_loss.item(),
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=100,
        help="learning rate for updating style translator",
    )
    ...
    parser.add_argument(
        "--force_save", action="store_true", help="this will save images for 50ipc"
    )
    ...
    parser.add_argument(
        "--single_channel",
        action="store_true",
        help="using single-channel but more basis",
    )
    parser.add_argument("--lambda_club_content", type=float, default=0.1)
    parser.add_argument("--lambda_likeli_content", type=float, default=1.0)
    parser.add_argument("--lambda_cls_content", type=float, default=1.0)
    parser.add_argument("--lambda_contrast_content", type=float, default=1.0)
