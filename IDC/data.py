...
class TensorDatasetFeature(torch.utils.data.Dataset):
...
class MultiEpochsDataLoader(torch.utils.data.DataLoader):
    ...
    def __iter__(self):
        for i in range(len(self)):
            data, target, feat = next(self.iterator)

            feat = feat.permute(1, 0, 2, 3, 4)

            # if i==0:
            #     print('===============MultiEpochsDataLoader==================')
            #     print('data, target, feat',data.shape, target.shape, feat.shape)
                # data, target, feat torch.Size([64, 3, 32, 32]) torch.Size([64]) torch.Size([1, 64, 128, 4, 4])
            if self.convert != None:
                data = self.convert(data)
                # feat = self.convert(feat)
            yield data, target, feat

class MultiEpochsDataLoaderVal(torch.utils.data.DataLoader):
    """Multi epochs data loader
    """
    def __init__(self,*args, **kwargs):
        super().__init__(*args, **kwargs)
