...
class SynthesizerFeature():
    ...
    # def decode_zoom_bound(self, img, target, feat, factor_max, bound=128):
    ...
    def decode(self, data, target, feature, bound=128):
        ...
        if self.factor > 1:
            ...
            # else:
            #     raise NotImplementedError(f'{decode_type} not supported!')
    ...
    for it in range(n_iter):
        ...
        for ot in range(args.inner_loop):
            ...
            for c in range(nclass):
                ...
                img_syn, lab_syn, _ = synset.sample(c, max_size=args.batch_syn_max)
                ts.stamp("data")
            ...
            if args.n_data > 0:
                ...
                pretrained_net = define_model(args, nclass).to(args.device) # get a random model
                pretrained_net.train()
                image_syn_train, label_syn_train = copy.deepcopy(synset.data.detach()), copy.deepcopy(synset.targets.detach())
                optimizer_pretrained = torch.optim.SGD(pretrained_net.parameters(), lr=args.lr) 
                optimizer_pretrained.zero_grad()
                criterion = nn.CrossEntropyLoss().to(args.device)
                train_epoch(args, loader_real, pretrained_net,criterion, optimizer_pretrained, n_data=args.n_data,
                                aug=aug_rand,
                                mixup=args.mixup_net)
