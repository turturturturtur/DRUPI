...
from train import define_model, train,train_with_features
...
def test_data_with_features(args,
              train_loader,
              val_loader,
              feature_criterion,
              test_resnet=False,
              ...
              repeat=1,
              logger=print,
              num_val=4,pooling_function=None,
    feat_loss=None,
    ...
    for model_fn in model_fn_ls:
        ...
        for _ in range(repeat):
            model = model_fn(args, args.nclass, logger=logger)
            # print("!!!!!")
            best_acc, acc = train_with_features(args, model, train_loader, val_loader,feature_criterion, logger=print,  pooling_function=pooling_function,feat_loss=feat_loss,feature_strategy=feature_strategy,pooling=args.poolFlag)
            best_acc_l.append(best_acc)
            acc_l.append(acc)
        logger(
            f'Repeat {repeat} => Best, last acc: {np.mean(best_acc_l):.1f} {np.mean(acc_l):.1f}\n')
