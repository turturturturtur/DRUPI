...
def main(args):
    ...
    if args.pix_init == "real":
        ...
        Initialized_Labels = Temp_net(image_syn_on_device, flat_param=Initialize_Labels_params) # 修正: 确保输入在正确设备
    ...
    for it in range(args.Iteration + 1):
        ...
        if it in eval_it_pool:
            ...
            for model_eval in model_eval_pool:
                ...
                for it_eval in range(args.num_eval):
                    ...
                    # 关键：将args传递给评估函数，以便内部的epoch函数可以访问到tta_mode
        ...
        starting_params_data = torch.cat([p.data.to(args.device).reshape(-1) for p in starting_params], 0) # 修正: 变量名
        student_params = [starting_params_data.requires_grad_(True)]
        ...
        for step in range(args.syn_steps):
            ...
            x_out = student_net(x, flat_param=forward_params)
            ce_loss = criterion(x_out, this_y)
            grad = torch.autograd.grad(ce_loss, student_params[-1], create_graph=True)[0]
            student_params.append(student_params[-1] - syn_lr * grad)

        param_loss = torch.nn.functional.mse_loss(student_params[-1], target_params, reduction="sum")
        param_dist = torch.nn.functional.mse_loss(starting_params_data, target_params, reduction="sum")
...
if __name__ == '__main__':
    ...
    # <<< 新增 TTA 参数 >>>
    parser.add_argument('--tta_mode', type=str, default=None, choices=['hflip'], help='Test-Time Augmentation mode. Currently only supports "hflip".')
