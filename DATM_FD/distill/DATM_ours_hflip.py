    ...
    for it in range(0, args.Iteration + 1):
        ...
        if it in eval_it_pool and (save_this_it or it % 1000 == 0):
            with torch.no_grad():
                ...
                torch.save(
                    feature_save.cpu(),
                    os.path.join(save_dir, "Normal", "features_{}.pt".format(it)),
                )
                torch.save(
                    label_syn.cpu(),
                ...
                if save_this_it:
                    ...
                    torch.save(
                        feature_save.cpu(),
                        os.path.join(save_dir, "Normal", "features_best.pt".format(it)),
                    )
                    torch.save(
                        label_syn.cpu(),
                ...
                if args.ipc < 50 or args.force_save:
                    ...
                    save_path = os.path.join(save_dir, "Normal",'Synthetic_Images')
                    ...
                    for clip_val in [2.5]:
                        ...
                        save_path = os.path.join(save_dir, "Normal",f'Clipped_Synthetic_Images/std_{clip_val}')
                        if not os.path.exists(save_path):
                            os.makedirs(os.path.join(save_path))
                        # 将图像数据从 GPU 转移到 CPU，并去除 NaN 值
                        image_data = torch.nan_to_num(grid.detach().cpu())
                    ...
                    if args.zca:
                        ...
                        save_path = os.path.join(save_dir, "Normal",'Reconstructed_Images')
                        if not os.path.exists(save_path):
                            os.makedirs(os.path.join(save_path))
                        # 将图像数据从 GPU 转移到 CPU，并去除 NaN 值
                        image_data = torch.nan_to_num(grid.detach().cpu())
                        ...
                        for clip_val in [2.5]:
                            ...
                            #     step=it,
                            # )
                            save_path = os.path.join(save_dir, "Normal",f'Clipped_Reconstructed_Images/std_{clip_val}')
                            if not os.path.exists(save_path):
                                os.makedirs(os.path.join(save_path))
                            # 将图像数据从 GPU 转移到 CPU，并去除 NaN 值
                            image_data = torch.nan_to_num(grid.detach().cpu())
        ...
        for step in range(args.syn_steps):
            ...
            #     print('syn_images:{},x:{}'.format(syn_images.shape, x.shape))
            #     print('aft this_feat',torch.mean(this_feat, dim=0).shape,torch.mean(this_feat, dim=1).shape)
            optimizer_feat.zero_grad()
            ...
            # if step == 0:
            #     print('aft shape',feature.shape,this_feat.shape)
...
if __name__ == "__main__":
    ...
    # TTA argument
    parser.add_argument('--tta_mode', type=str, default=None, choices=['hflip', 'fivecrop', 'hflip5'], help='TTA mode: horizontal flip (hflip), 5-crop (fivecrop), or hflip+5-crop (hflip5)')
    #
