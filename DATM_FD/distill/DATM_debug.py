...
def main(args):
    ...
    if args.feat_opt == "SGD":
        optimizer_feat = torch.optim.SGD(
            ...
            lr=args.lr_feat,
            momentum=0.5,
        )  # optimizer_feat for synthetic features
    else:
    ...
    # if args.pix_init == "samples_predicted_correctly":
        # if cfg.Initialize_Label_With_Another_Model:
        #     Temp_net = get_network(
        #         args.Initialize_Label_Model, channel, num_classes, im_size, dist=False
        #     ).to(
        #         device
        #     )  # get a random model
        # else:
        ...
        #         temp_logits = None
        #         for select_times in range(
        #             (len(image_syn) + batch_size - 1) // batch_size
        #         ):
        #             current_data_batch = (
        #                 image_syn[
        #                     batch_size * select_times : batch_size * (select_times + 1)
        #                 ]
        #                 .detach()
        #                 .to(device)
        #             )
        #             Temp_params = j[args.Label_Model_Timestamp]
        #             Initialize_Labels_params = torch.cat(
        #                 [p.data.to(args.device).reshape(-1) for p in Temp_params], 0
        #             )
        #             if args.distributed:
        #                 Initialize_Labels_params = Initialize_Labels_params.unsqueeze(
        #                     0
        #                 ).expand(torch.cuda.device_count(), -1)
        #             Initialized_Labels = Temp_net(
        #                 current_data_batch, flat_param=Initialize_Labels_params
        #             )
        #             if temp_logits == None:
        #                 temp_logits = Initialized_Labels.detach()
        #             else:
        #                 temp_logits = torch.cat(
        #                     (temp_logits, Initialized_Labels.detach()), 0
        #                 )
        #         logits.append(temp_logits.detach().cpu())
        # logits_tensor = torch.stack(logits)
        # # print("!!!!!",torch.isnan(logits_tensor).any())
        ...
        # print("logits_tensor",logits_tensor)
        # contains_zero = torch.any(torch.eq(logits_tensor, 0))
        # print(contains_zero)
        # print("mask",mask)
        # contains_false = torch.any(logits_tensor == False)
        # print(contains_false)
        # correct_logits = logits_tensor * mask.float()
        # print("correct_logits",correct_logits)
        # contains_zero = torch.any(torch.eq(correct_logits, 0))
        # print(contains_zero)
        # correct_logits_per_model = correct_logits.sum(dim=0)
        # contains_zero = torch.any(torch.eq(correct_logits_per_model, 0))
        # print(contains_zero)
        # print("correct_logits_per_model",correct_logits_per_model)
        # num_correct_images_per_model = correct_predictions.sum(dim=0, dtype=torch.float)
        # # print("correct_logits_per_model",correct_logits_per_model)
        # # print("num_correct_images_per_model",num_correct_images_per_model)
        # # print("num_correct_images_per_model shape ",num_correct_images_per_model.shape)
        # average_logits_per_image = (
        #     correct_logits_per_model / num_correct_images_per_model.unsqueeze(1)
        # )
        # Initialized_Labels = average_logits_per_image
        # print("Initialized_Labels:",Initialized_Labels)
    ...
    # acc = np.sum(
    #     np.equal(
    #         np.argmax(Initialized_Labels.cpu().data.numpy(), axis=-1),
    #         label_syn.cpu().data.numpy(),
    #     )
    # )
    ...
    # label_syn = (
    #     copy.deepcopy(Initialized_Labels.detach()).to(args.device).requires_grad_(True)
    # )
    ...
    for it in range(0, args.Iteration + 1):
        ...
        # print("student_params:",student_params)
        ...
        # print("syn_images:",syn_images.shape)
        ...
        for step in range(args.syn_steps):
            ...
            # print("shapes", syn_images.shape, y_hat.shape, syn_feat.shape)
            # print("indices", these_indices.shape)
            # print("indices", these_indices)
            x = syn_images[these_indices]
            this_y = y_hat[these_indices]
            # print("Y:",y_hat)
            # this_feat = syn_feat[:, these_indices]
            # syn_feat (1,10,3,32 32)
            this_feat = syn_feat[:,these_indices]
            # print("shapes", x.shape, this_y.shape, this_feat.shape)
            # print("=========================")
            # feature_syn: torch.Size([1, 100, 128, 4, 4])
            # syn_images: torch.Size([100, 3, 32, 32])
            # shapes torch.Size([100, 3, 32, 32]) torch.Size([100, 10]) torch.Size([1, 100, 128, 4, 4])
            # indices torch.Size([100])
            # syn_feat: torch.Size([1, 100, 128, 4, 4])




            # print("======================")
            # print("syn_feat:",syn_feat.shape)
            # print("this_feat:",this_feat.shape)
            # this_feat = this_feat.squeeze(0)
            # this_feat (10,3,32 32)

            # print('this_feat_before',this_feat.shape)
            optimizer_feat.zero_grad()
            ...
            # print('this_feat_after',this_feat.shape)
            ...
            # print("=========================")
            # print('feature,this_feat',feature.shape,this_feat.shape)
            ...
            # print("this_feat",this_feat)
            # print("loss_reg",loss_reg)
            loss_syn = loss_ce + lbd * loss_reg

            # print("loss_syn",loss_syn)
            # print("student_params[-1]",student_params[-1])
            ...
            # update features
            if isinstance(student_net, torch.nn.DataParallel):
                # print("this_feat_fixed:",this_feat_fixed.shape)
                ...
                # print("output_feat_syn:",output_feat_syn.shape)
                output_feat_syn = student_net.module.classifier(
                    this_feat_fixed.view(this_feat.size(0), -1)
                )
                # print("output_feat_syn:",output_feat_syn.shape)
                ...
                output_feat_syn = student_net.classifier(
                    this_feat_fixed.view(this_feat.size(0), -1)
                )

            loss_feat_step = criterion(output_feat_syn, this_y)
            # print("output_feat_syn:",output_feat_syn.shape)
            # print("y:",this_y.shape)
...
if __name__ == "__main__":
    ...
    args = parser.parse_args()
    model_args = f'{args.dataset}_{args.ipc}ipc_[{args.feat_metric}{args.lbd}_pool{args.pooling}_layer{args.layer_idx}_CE{args.feat_lbd}_opt{args.feat_opt}_nfeat{args.n_feat}_use{args.use_feature}_norm{args.feat_norm}]_{args.img_method}'

    log_file = os.path.join(args.save_path, f'{model_args}.txt')
