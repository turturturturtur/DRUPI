...
def main(args):
    ...
    best_acc_wo = {m: 0 for m in model_eval_pool}
    best_std_wo = {m: 0 for m in model_eval_pool}
    ...
    for model_eval in model_eval_pool:
        ...
        for it_eval in range(args.num_eval):
            ...
            #
            accs_test_wo.append(acc_test)
            accs_train_wo.append(acc_train)

        accs_test_wo = np.array(accs_test_wo)
        accs_train_wo = np.array(accs_train_wo)
        acc_test_mean_wo= np.mean(accs_test_wo)
        acc_test_std_wo = np.std(accs_test_wo)

        # if acc_test_mean_wo > best_acc_wo[model_eval]:
        #     best_acc_wo[model_eval] = acc_test_mean
        #     best_std_wo[model_eval] = acc_test_std
        #     save_this_it = True
        print(
            "Evaluate %d random %s, mean = %.4f std = %.4f strategy = %s \n-------------------------"
            % (len(accs_test_wo), model_eval, acc_test_mean_wo, acc_test_std_wo,feature_strategy)
        )

        print("==================Evaluate with feature!=======================")
        for it_eval in range(args.num_eval):
            if args.parall_eva == False:
                device = torch.device("cuda:0")
                net_eval = get_network(
