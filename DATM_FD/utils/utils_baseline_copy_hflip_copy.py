...
#     else:
#         return None, acc_train, acc_test


def evaluate_synset_w_feature(it_eval, net, images_train, labels_train,feature_syn_eval, testloader, args, criterion_feature, return_loss=False, texture=False, train_criterion=None, Preciser_Scheduler=False, type=1,pooling_function=None, feat_loss=None, feature_strategy=None):
    device = args.device
    net = net.to(device)
    images_train.to(device)
    labels_train.to(device)
    ...
    for ep in tqdm.tqdm(range(Epoch+1)):
        loss_train, acc_train = epoch_with_features(
            "train", trainloader, net, optimizer, train_criterion, criterion_feature,
            args, aug=True, lbd=args.lbd, pooling_function=pooling_function,
            layer_idx=args.layer_idx, feat_loss=feat_loss,
            feature_strategy=feature_strategy, texture=texture, If_Float=If_Float,
        )
    ...
    time_train = time.time() - start
    final_loss_train = loss_train_list[-1] if loss_train_list else -1
    final_acc_train = acc_train_list[-1] if acc_train_list else -1
    print('%s Evaluate_%02d: epoch = %04d train time = %d s train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), it_eval, Epoch, int(time_train), final_loss_train, final_acc_train, acc_test))

    if return_loss:
        return net, acc_train_list, acc_test, loss_train_list, loss_test
    else:
        # 正确返回包含所有训练精度的列表
        return None, acc_train_list, acc_test
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug, texture=False, If_Float = False):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        # --- START of TTA MODIFICATION ---
        if mode == 'test' and hasattr(args, 'tta_mode') and args.tta_mode is not None:
            if args.tta_mode == 'hflip':
                # Original and horizontally flipped
                output_orig = net(img)
                output_flipped = net(torch.flip(img, [3]))
                output = (output_orig + output_flipped) / 2.0
            
            elif args.tta_mode == 'fivecrop' or args.tta_mode == 'hflip5':
                # This is a simplified implementation of 5-crop TTA. A more robust version 
                # might require changes to the data loading pipeline to handle images of 
                # different sizes. Here we crop to 90% and resize back up.
                orig_h, orig_w = img.shape[2], img.shape[3]
                crop_h, crop_w = int(orig_h * 0.9), int(orig_w * 0.9)
                
                # Manually create crops for the whole batch
                crops = [
                    img[..., :crop_h, :crop_w],  # Top-left
                    img[..., :crop_h, -crop_w:], # Top-right
                    img[..., -crop_h:, :crop_w], # Bottom-left
                    img[..., -crop_h:, -crop_w:],# Bottom-right
                    # Center crop
                    img[..., (orig_h - crop_h) // 2 : (orig_h + crop_h) // 2, (orig_w - crop_w) // 2 : (orig_w + crop_w) // 2] 
                ]
                
                # Resize crops back to original size for the network
                resized_crops = [F.interpolate(c, size=(orig_h, orig_w)) for c in crops]
                
                if args.tta_mode == 'hflip5':
                    # Add horizontally flipped versions of each crop
                    flipped_crops = [torch.flip(rc, [3]) for rc in resized_crops]
                    resized_crops.extend(flipped_crops)
                
                # Process all augmented images in one go
                all_crops_batch = torch.cat(resized_crops, dim=0)
                all_outputs = net(all_crops_batch)

                # Reshape back and average the predictions
                num_augs = len(resized_crops)
                # Reshape to [num_augs, batch_size, num_classes]
                all_outputs = all_outputs.view(num_augs, n_b, -1)
                output = all_outputs.mean(dim=0) # Average across augmentations
            else:
                 output = net(img) # Fallback for unknown TTA mode
        else:
            output = net(img) # Default behavior for training or no TTA
        # --- END of TTA MODIFICATION ---
        ...
        if param.aug_mode == 'M': # original
            for p in strategy.split('_'):
                for f in AUGMENT_FNS[p]:
                    x = f(x, param)
                    # print(f'应用 {f.__name__}后的形状:{x.shape}')
...
def rand_rotate(x, param):
    ...
    theta_rad = (torch.rand(x.shape[0]) - 0.5) * 2 * ratio / 180 * float(np.pi)
    
    # Create affine transformation matrices
    theta_cos = torch.cos(theta_rad)
    theta_sin = torch.sin(theta_rad)
    theta = torch.stack([theta_cos, -theta_sin, torch.zeros_like(theta_rad),
                         theta_sin, theta_cos, torch.zeros_like(theta_rad)], dim=1)
    theta = theta.view(-1, 2, 3)  # reshape to (batch_size, 2, 3)
...
def rand_cutout(x, param):
    ...
    set_seed_DiffAug(param)
    offset_x = torch.randint(0, x.size(2) - cutout_size[0] + 1, size=(x.size(0), 1, 1), device=x.device)
    set_seed_DiffAug(param)
    offset_y = torch.randint(0, x.size(3) - cutout_size[1] + 1, size=(x.size(0), 1, 1), device=x.device)
