...
# 
        ...
        n_b = lab.shape[0]
        # --- TTA LOGIC START ---
        # Only activate in test mode and if tta_mode is set to 'hflip'
        if mode == 'test' and hasattr(args, 'tta_mode') and args.tta_mode == 'hflip':
            output_orig = net(img)
            output_flipped = net(torch.flip(img, [3])) # Horizontal flip
