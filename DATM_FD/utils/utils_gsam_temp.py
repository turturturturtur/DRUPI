...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        if args.zca:
            ...
            transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        # 数据预处理  下面的代码只需运行一次即可
        val_path = data_path + '/tiny-imagenet-200/val/val_annotations.txt'
        target_folder = data_path + '/tiny-imagenet-200/val/'
        ...
        glob_path = target_folder + 'images/*'
        ...
        rmdir_path = target_folder + 'images'
        rmdir(rmdir_path)
