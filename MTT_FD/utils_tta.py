...
def tta_logits(net: nn.Module, x: torch.Tensor, mode: str = 'hflip') -> torch.Tensor:
    ...
    - 'hflip'   : original + horizontal flip, average logits
    - 'fivecrop': five-crop (TL/TR/BL/BR/Center), average logits (crops resized back to original HxW)
    - 'hflip5'  : five-crop + hflip on each crop (10 views), average logits
    """
    ...
    if mode == 'hflip':
        out = (_forward(x) + _forward(torch.flip(x, dims=[3]))) / 2.0

    elif mode in ('fivecrop', 'hflip5'):
        N, C, H, W = x.shape
        # keep 87.5% area like common val transforms
        ...
        views = []
        for i in range(N):
            tl,tr,bl,br,cc = TF.five_crop(x[i], (crop_h, crop_w))
            crops = torch.stack([tl,tr,bl,br,cc], dim=0)  # (5, C, h, w)
            views.append(crops)
        views = torch.cat(views, dim=0)  # (N*5, C, h, w)

        # resize crops back to original size to be safe for models with fixed heads
        if views.shape[-2:] != (H, W):
            views = F.interpolate(views, size=(H, W), mode="bilinear", align_corners=False)

        logits5 = _forward(views).view(N, 5, -1).mean(1)  # (N, num_classes)
        outs = [logits5]
        if mode == 'hflip5':
            views_flip = torch.flip(views, dims=[3])
            logits5f = _forward(views_flip).view(N, 5, -1).mean(1)
            outs.append(logits5f)
