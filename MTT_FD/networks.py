...
class MLP(nn.Module):
    def __init__(self, channel, num_classes):
        ...
        input_size = 28 * 28 * 1 if channel == 1 else 32 * 32 * 3

        self.fc_1 = nn.Linear(input_size, 128)
        self.fc_2 = nn.Linear(128, 128 * 4 * 4)
        self.fc_3 = nn.Linear(128 * 4 * 4, num_classes)
    ...
    def get_output_from_features(self, f, layer_idx=None):
        f = f.view(f.size(0), -1)  # Flatten the feature map

        if layer_idx is None:
            # If no specific layer is mentioned, apply the classifier
        ...
        for idx, layer in enumerate(self.features):
            out = layer(out)
            if idx == layer_idx:
                return out.view(out.size(0), -1)  # Return the output of the specific layer

        # If layer_idx exceeds the total number of layers in features, return final classifier output
        return self.classifier(out)

    def get_features(self, x, layer_idx=None):
        out = x.view(x.size(0), -1)  # Flatten input
        if layer_idx is None:
            return self.features(out).reshape(out.size(0), 128, 4, 4)  # Return features up to the final layer

        # Loop through the layers and stop at the requested index
        for idx, layer in enumerate(self.features):
            out = layer(out)
            if idx == layer_idx:
                return out

        return out.reshape(out.size(0), 128, 4, 4)  # Return the output if layer_idx is not specified or exceeds the layers
...
class LeNet(nn.Module):
    def __init__(self, channel, num_classes):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(channel, 32, kernel_size=3, padding=3 if channel == 1  else 1), 
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(32, 64, kernel_size=3, padding=3 if channel == 1  else 1), 
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(64, 128, kernel_size=3, padding=3 if channel == 1  else 1), 
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )
        self.fc_1 = nn.Linear(128 * 4 * 4, 120)
    ...
    def get_output_from_features(self, f, layer_idx=None):
        ...
        if layer_idx == None:           
            f = f.view(f.shape[0], -1)
            classifier=nn.Linear(f.shape[1],self.num_classes).to(f.device)
            return classifier(f).to(f.device)
        out = f
        tgt_idx = 0
        for layer in self.features:
            if isinstance(layer, nn.Conv2d):
            ...
            if tgt_idx > layer_idx:
                out = layer(out)
        out = out.view(out.shape[0],-1)
        classifier=nn.Linear(out.shape[1],self.num_classes)
        out = classifier(out)
        return out
...
class AlexNet(nn.Module):
    def __init__(self, channel, num_classes):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(channel, 64, kernel_size=3, padding=3 if channel == 1  else 1),  
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),  
            nn.Conv2d(64, 128, kernel_size=3, padding=3 if channel == 1  else 1),  
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),  
            nn.Conv2d(128, 256, kernel_size=3, padding=3 if channel == 1  else 1),  
            nn.ReLU(inplace=True),
            nn.Conv2d(256, 128, kernel_size=3, padding=3 if channel == 1  else 1), 
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2) 
        )
    ...
    def get_output_from_features(self, f, layer_idx=None):
        ...
        for layer in self.features:
            ...
            if tgt_idx > layer_idx:
                out = layer(out)
        out = out.view(out.shape[0], -1)
        classifier = nn.Linear(out.shape[1], self.num_classes).to(out.device)
        out = classifier(out)
        return out
...
cfg_vgg = {
    ...
    'VGG11': [64, 'M', 128, 'M', 256, 256, 'M', 256, 128],  
    'VGG13': [64, 64, 'M', 128, 128, 'M', 256, 256, 'M', 256, 128, 'M'], 
    'VGG16': [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 256, 128, 'M'],  
    'VGG19': [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 256, 'M', 256, 128, 'M']  
}
class VGG(nn.Module):
    def __init__(self, vgg_name, channel, num_classes, norm='instancenorm'):
        super(VGG, self).__init__()
        self.channel = channel
        self.features = self._make_layers(cfg_vgg[vgg_name], norm, channel)
    ...
    def _make_layers(self, cfg, norm, channel):
        layers = []
        ...
        for ic, x in enumerate(cfg):
            if x == 'M':
                ...
                layers += [nn.Conv2d(in_channels, x, kernel_size=3, padding=3 if channel == 1 else 1),
...
class ResNet_AP(nn.Module):
    def __init__(self, block, num_blocks, channel=3, num_classes=10, norm='instancenorm'):
        ...
        self.features = nn.Sequential(
            ...
            self._make_layer(block, 128, num_blocks[3], stride=2)
        )
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, channel=3, num_classes=10, norm='instancenorm'):
        ...
        self.conv1 = nn.Conv2d(channel, 64, kernel_size=3, stride=1, padding=3, bias=False)
        self.bn1 = nn.GroupNorm(64, 64, affine=True) if self.norm == 'instancenorm' else nn.BatchNorm2d(64)
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=1)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=1)
        self.layer4 = self._make_layer(block, 128, num_blocks[3], stride=1)
        self.classifier = nn.Linear(128 * 4 * 4 * block.expansion, num_classes)
        self.features = nn.Sequential(
            self.conv1, self.bn1, nn.ReLU(inplace=True), self.layer1, self.layer2,
            nn.AvgPool2d(kernel_size=4, stride=4), self.layer3, self.layer4, nn.AvgPool2d(kernel_size=2, stride=2)
        )
        self.num_classes=num_classes
    ...
    def get_output_from_features(self, f, layer_idx=None):
        ...
        if layer_idx == None:           
            f = f.view(f.shape[0], -1)
            classifier=nn.Linear(f.shape[1],self.num_classes)
            return classifier(f)
        out = f
        tgt_idx = 0
        for layer in self.features:
